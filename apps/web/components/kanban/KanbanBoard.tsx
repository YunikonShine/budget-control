...
type Card = { id: string; title: string; order: number; columnId: string }
type Column = { id: string; name: string; cards: Card[] }

export default function KanbanBoard({ projectId = "1" }: { projectId?: string }) {
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    setColumns((prev) => {
      ...
      if (src.id === dst.id) {
        ...
        const overIsCard = overId ? src.cards.some((c) => c.id === overId) : false
        if (!overIsCard) {
        ...
        const overIsCard = overId ? dst.cards.some((c) => c.id === overId) : false
        const overIndex = overIsCard ? dst.cards.findIndex((c) => c.id === overId) : -1
        const activeRect: any = (active as any).rect?.current?.translated
        const overRect: any = (over as any).rect
        const isBelow = activeRect && overRect
          ? activeRect.top > overRect.top + overRect.height / 2
          : false
        const desiredIndex = overIsCard ? overIndex + (isBelow ? 1 : 0) : dst.cards.length

        // If preview already moved the card, reposition it precisely; otherwise move from source
        const currentIdxInDst = dst.cards.findIndex((c) => c.id === activeCardId)
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/cards/move`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        const br = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/columns`)
  ...
  return (
    <div className={styles.board}>
      ...
      >
        {columns.map((col) => (
          <Column key={col.id} column={col} hoveredColumnId={hoveredColumnId} />
        ))}

        <DragOverlay>
          {activeCard ? (
            <div className={`${styles.card} ${styles.dragOverlay}`} style={{ width: 320 }}>
...
function Column({ column, hoveredColumnId }: { column: Column; hoveredColumnId: string | null }) {
  ...
  <div ref={setNodeRef} className={`${styles.columnList} ${hoveredColumnId === column.id ? styles.columnListOver : ""}` }>
        <SortableContext items={column.cards.map((c) => c.id)} strategy={verticalListSortingStrategy}>
          {column.cards.map((card) => (
            <SortableCard key={card.id} id={card.id} title={card.title} />
          ))}
        </SortableContext>
        {column.cards.length === 0 && <div className={styles.emptyState}>Sem cards</div>}
      </div>
    </div>
  )
}
